     ...
     */
            public function prepareImages(array $rowData, array &$imagesData): void 
        {
            if (empty($rowData['images'])) {
                Log::info("Nessuna immagine da importare", ['sku' => $rowData['sku']]);
                return;
            }
        
            // Assicurati che la directory esista
            $uploadPath = storage_path('app/public/import/images');
            if (!file_exists($uploadPath)) {
                mkdir($uploadPath, 0755, true);
                Log::info("Creata directory", ['path' => $uploadPath]);
            }
            ...
            foreach ($imageNames as $key => $image) {
                try {
                    $filename = $rowData['sku'] . '_' . $key . '.jpg';
                    $relativePath = 'import/images/' . $filename;
                    $absolutePath = storage_path('app/public/' . $relativePath);
                    
                    Log::info("Debug percorsi", [
                        'sku' => $rowData['sku'],
                    ...
                    if (!$content) {
                        $errorCount++;
                        Log::error("Errore download immagine", [
                            'sku' => $rowData['sku'],
                            'image_url' => $image,
                            'error' => error_get_last()['message'] ?? 'Download fallito'
                        ]);
                        continue;
                    }
        
                    // Salva usando il percorso relativo per Storage
                    if (!Storage::put('public/' . $relativePath, $content)) {
                        $errorCount++;
                        Log::error("Errore salvataggio immagine", [
                            'sku' => $rowData['sku'],
                            'path' => $absolutePath
                        ]);
                        continue;
